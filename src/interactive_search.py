...
class InteractiveSearch:
    ...
    def setup(self):
        ...
        if os.path.exists(idx_file):
            ...
            self.sys.build_index(
                ...
                use_stemming=pp.get('use_stemming', False),
    ...
    def search(self, q):
        ...
        if not r.get('results'):
            return
        for i, doc in enumerate(r['results'][:self.sys.final_top_k], 1):
            print(f"{i}. {doc['heading']}")
            print(f"   {doc['article']}")
            print(f"   {doc['date']} | {doc['news_type']}")
            print(f"   score={doc['hybrid_score']:.3f} (neural={doc['neural_score']:.3f}) [id={doc['doc_id']}]")
            print()
